...
public class Level2 {
	...
	int difficulty;
	boolean isSetDiff;
	public Level2(MainCharacter mm,int p,int diff){
		difficulty = diff;
		isSetDiff = true;
		finished = false;
		points = p;
		...
		offsetX = 0;
		try {
			layer1 = ImageIO.read(new File("src\\game\\level2\\layer1.png"));
		...
		for(int i=0;i<enemyNr;i++){
			if(rand.nextInt(2)==0)
				enemies[i] = new Enemy("eagle",difficulty+1,8,800+rand.nextInt(600),rand.nextInt(550));
			else enemies[i] = new Enemy("redbird",difficulty+1,8,800+rand.nextInt(600),rand.nextInt(550));
		}
		mb = new MiniBoss[2];
		...
		b = new Boss("bosslvl2",200,7,5,800+rand.nextInt(200),100,2);
	}
			...
			if(timer<170){
				...
				if(timer < 470){
					...
					if(b.bulletCollision(m.getX(), m.getY(), m.getWidth(), m.getHeight()))
						m.boom(difficulty);
					if(m.bulletCollision(b.getX()+170, b.getY(), b.getWidth(), b.getHeight()))
						b.boom(difficulty);
					b.move();
				}	else if(ftime < 100){
